...
- 生成用例前先拆解原子化 `测试点矩阵`，分析功能行为、影响面、正反例、边界值、组合场景、权限差异、配置差异、模型/集群差异和自动化验证方式
- `测试点矩阵` 不能只是高层分类；权限、配置、Pipe/同步、集群、性能、元数据展示等复杂能力必须按角色、操作、范围、开关、模型、拓扑、源端/接收端等维度继续拆细
- 根据测试点矩阵生成详细用例，不把“需求句子被提到”当成有效覆盖
- 如果测试点涉及写入、删除、查询、同步、权限、配置、重启或性能，必须执行对应动作并验证真实结果
- `SHOW`、`DESC`、`information_schema` 只用于证明元数据行为，不能代替数据路径验证
...
这个 skill 是通用 IoTDB/TimechoDB SQL 测试设计流水线，不是某个功能的专用模板。每次输入新的需求项时，都应重新分析需求、设计文档和官方手册，拆出可执行、可验证、可追踪的测试点，再生成详细用例。

生成用例时必须遵守：

- 一个测试点只验证一个可观察规则，不能把多个细节压缩成“权限控制”“同步验证”“配置测试”这类大项。
- 每个功能声明都要能被 SQL、远端命令、benchmark、日志或产物证明；只提到需求句子不算覆盖。
- 涉及写入、删除、更新、查询、同步、权限、配置、重启、性能或工具行为时，必须执行真实动作并校验结果。
- 有成功路径时应考虑失败路径；有开关/默认值时应考虑开启、关闭、显式、默认；有权限时应考虑有权限、无权限、不同授权范围。
- 不能机械地 1 个测试点生成 1 条用例；测试点矩阵之后必须生成 `测试点展开表`，把正向、反向、边界、权限、配置、模型、集群、性能等子场景展开成具体用例。
- 不能随意把 Pipe、集群、性能、审计、大数据等内容标成“后续专项”。如果确实无法自动化，必须写明阻塞原因、触发动作、所需环境和证据计划。
- Markdown 用例必须使用详细表格字段，包含 `用例名称`、`一级分类`、`二级分类`、`操作数据`、`清理动作`、`测试结果`、`截图` 等信息。
...
日常实验推荐只写触发句、环境信息和需求。官网检索、测试点矩阵拆解、Markdown 生成、`.run` 生成、SQL-test 配置、setup/test、结果对比、产物拉回和报告生成都由 skill 自动执行。
...
- 详细用例前必须先有测试点矩阵；测试点矩阵要覆盖正向、反向、边界、组合、权限、配置、模型、集群、同步、系统表和性能等相关影响面。
- 用例标题、授权、SQL 动作和预期结果必须一致。标题写 `INSERT/DELETE/SELECT` 权限时，步骤必须授权并执行对应动作，不能用无关的 `DROP` 或纯元数据检查替代。
- 只有直接执行并断言对应行为，才算覆盖该测试点；只写概括性步骤或只做 `SHOW`/`DESC` 不算覆盖数据行为。
